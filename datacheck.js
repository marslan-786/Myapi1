export default async function handler(req, res) {
  ...
  try {
    const fetchUrl = `https://pakdatabase.site/api/search.php?username=${username}&password=${password}&search_term=${search_term}`;
    const response = await fetch(fetchUrl);
    const data = await response.json();

    // Get first key (network name, like 'jazz', 'zong', etc.)
